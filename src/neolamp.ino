...
#define NEO_SWITCH_ID     "61edab2b5237d163c3054056"
...
#define NEO_PIN   14  // GPIO for NeoPixel
#define NEO_COUNT 16  // NeoPixel count

Adafruit_NeoPixel strip(NEO_COUNT, NEO_PIN, NEO_GRB + NEO_KHZ800);

bool myPowerState = false;
bool NeoPowerState = true;
...
bool onPowerStateNeo(const String &deviceId, bool &state) {
  Serial.printf("NeoPixel Device %s turned %s (via SinricPro) \r\n", deviceId.c_str(), state?"on":"off");
  NeoPowerState = state;
  if (NeoPowerState) {     // flip myPowerState: if it was true, set it to false, vice versa
      //ON
      //NeoPowerState = false;
      //rainbow(10);
  ...
  //digitalWrite(NEO_PIN, myPowerState?LOW:HIGH);
  return true; // request handled properly
}
...
void handleTouch() {
  ...
  if(digitalRead(TOUCH_PIN) == LOW && nowMillis - lastTouch > 1000) {
    if(NeoPowerState) {
      NeoPowerState = false;
    } else {
      NeoPowerState = true;
    ...
    //digitalWrite(NEO_PIN, NeoPowerState?LOW:HIGH);

    // get Switch device back
    SinricProSwitch& mySwitchNeo = SinricPro[NEO_SWITCH_ID];
    // send powerstate event
    mySwitchNeo.sendPowerStateEvent(NeoPowerState); // send the new powerState to SinricPro server
    Serial.printf("Device %s turned %s (manually via touchsensor)\r\n", mySwitchNeo.getDeviceId().c_str(), NeoPowerState?"on":"off");

    lastTouch = nowMillis;  // update last button press variable
  }
}
...
void setupSinricPro() {
  ...
  //NeoPixel
  SinricProSwitch& mySwitchNeo = SinricPro[NEO_SWITCH_ID];
  mySwitchNeo.onPowerState(onPowerStateNeo);
...
void setup() {
  ...
  pinMode(TOUCH_PIN, INPUT_PULLUP); // GPIO 4 as input, pulled high
  pinMode(LED_PIN, OUTPUT); // define LED GPIO as output
  pinMode(NEO_PIN, OUTPUT); // define LED GPIO as output
  digitalWrite(LED_PIN, HIGH); // turn off LED on bootup
  digitalWrite(NEO_PIN, HIGH); // turn off LED on bootup
...
void loop() {
  if(millis() > (lastPass + 1000)) {
    tread = digitalRead(TOUCH_PIN);
    Serial.printf("TouchPin: %d\r\n", tread);
    Serial.printf("NeoPowerState LOOP: %o\r\n", NeoPowerState);
    lastPass = millis();
  }
  handleButtonPress();
  handleTouch();
  SinricPro.handle();
  //if (NeoPowerState == false) rainbow(10);
}
